...
class CvDNode:
    ...
    def __init__(self, tree=None, base=2, ent=None,
        ...
        if self.tree is not None:
            self.continuous = tree.whether_continuous
    ...
    def stop1(self, eps):
        if (
                            self._x.shape[1] == 0 or (self.ent is not None and self.ent <= eps)
    ...
    def prune(self):
        """
        在决策树的剪枝算法中被调用，将node变为leaf
        :return:
        """
        # 判断节点类别
        self.category = self.get_category()

        # 把该node的leafs中的leaf从该node的列祖列宗中的leafs中删除
        # 把该node存进列祖列宗的leafs中
    ...
    def fit(self, x, y, sample_weight, feature_bound=None, eps=1e-8):
        ...
        # 判断该 node 是否应该被当做 leaf；
        # 若是，则 return，
        # 否则继续往下走算出各维度的条件熵，记录下最好的条件熵、信息增益和此时关注的数据维度
        # 根据信息增益判断是否终止
        if self.stop1(eps):
            return
        ...
        if self.is_root:
            if self.criterion == "gini":
                self.ent = _cluster.gini()
            else:
                self.ent = _cluster.ent()
        _max_gain, _ent_lst = 0, []
        ...
        for feat in tmp_feats:
            ...
            if self.continuous[feat]:
                ...
                if self.is_cart:
                    ...
                    _set = None
            if self.is_cart or self.continuous[feat]:
                for tar in _set:
                    _tmp_gain, _tmp_ent_lst = _cluster.bin_info_gain(
                        feat, tar, criterion=self.criterion, get_ent_lst=True, continuous=self.continuous[feat])
                    if _tmp_gain > _max_gain:
                        (_max_gain, _ent_lst), _max_feature, _max_tar = (_tmp_gain, _tmp_ent_lst), feat, tar
            else:
                _tmp_gain, _tmp_ent_lst = _cluster.info_gain(
                    feat, self.criterion, True, self.tree.feature_sets[feat])
                if _tmp_gain > _max_gain:
                    (_max_gain, _ent_lst), _max_feature = (_tmp_gain, _tmp_ent_lst), feat
        if self.stop2(_max_gain, eps):
            return

        # 根据所选的数据维度的各个特征把数据集切分成几份，分别喂给新的
        # node、递归，同时把这些node记录在自己的children里面
        self.feature_dim = _max_feature
        if self.is_cart or self.continuous[_max_feature]:
            self.tar = _max_tar
            self._gen_children(_ent_lst, feature_bound)
    ...
    def _gen_children(self, _ent_lst, feature_bound):
        feat, tar = self.feature_dim, self.tar
        self.is_continuous = continuous = self.continuous[feat]
        ...
        if self.is_cart or continuous:
            ...
            for _feat, side, _ent in zip(_feats, ["left_child", "right_child"], _ent_lst):
                _new_node = self.__class__(
                    self.tree, self.base, ent=_ent,
            ...
            for feat, _ent in zip(self.tree.feature_sets[self.feature_dim], _ent_lst):
                _feat_mask = features == feat
                tmp_x = self._x[_feat_mask, ...]
                if len(tmp_x) == 0:
                    continue
                _new_node = self.__class__(
                    tree=self.tree, base=self.base, ent=_ent,
    ...
    def cost(self, pruned=False):
        if not pruned:
            return sum([leaf["ent"] * len(leaf["y"]) for leaf in self.leafs.values()])
        return self.ent * len(self._y)
